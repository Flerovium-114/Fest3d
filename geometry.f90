...
use openacc
    ...
    contains
        ...
        subroutine allocate_memory(cells, Ifaces, Jfaces, Kfaces)
            ...
            ! Use OpenACC to allocate memory on the GPU
            !$acc enter data create(cells, Ifaces, Jfaces, Kfaces)
	    ...
	    ! Use OpenACC to update the GPU memory with the allocated data
            !$acc update device(cells, Ifaces, Jfaces, Kfaces)
        ...
        subroutine normalize_face_normals(Ifaces, Jfaces, Kfaces, bc)
            ...
            !$acc parallel loop vector collapse(3) present(Ifaces)
            ...
            !$acc parallel loop vector collapse(3) present(Jfaces)
            ...
            !$acc parallel loop vector collapse(3) present(Kfaces)
        ...
        subroutine compute_face_area_vectors(Ifaces, Jfaces, Kfaces, nodes)
            ...
            !$acc parallel loop collapse(3) present(Ifaces, Jfaces, Kfaces, nodes) 
        ...
        subroutine compute_volumes(cells, nodes)
            ...
            !$acc parallel loop collapse(3) present(p_list, nodes, cells) 
        ...
        subroutine compute_ghost_cell_centroid(cells, nodes)
          ...
          !$acc parallel loop collapse(3) present(cells, nodes) 
